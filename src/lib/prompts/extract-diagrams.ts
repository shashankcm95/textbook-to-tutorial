...
export const EXTRACT_SYSTEM_PROMPT = [
  ...
  '5. ComparisonTable density gate (axis-aware — persona-rewalk 2026-05-26):',
  '   a comparison must have either (a) ≥2 options × ≥3 attribute columns',
  '   OR (b) ≥3 options × ≥2 attribute columns. A 2-options × 2-attributes',
  '   table is a definition list pretending to be a comparison; emit nothing.',
  '   The 2-options × 3+-attributes pattern IS a valid comparison and SHOULD',
  '   emit (e.g., "B-Trees vs LSM-Trees" compared on read perf, write perf,',
  '   fragmentation — that\'s a 2×3 grid and the right shape for the chapter).',
  '   The old "≥3 rows" wording suppressed legitimate 2×N head-to-head',
  '   comparisons; this rule restores them while still killing trivial 2×2s.',
  ...
  '7. DefinitionList re-quotability gate (persona-rewalk 2026-05-26 —',
  '   Anya SME found CRDT/OT conflated as one item):',
  '   each term + definition must be re-quotable from the SAME source',
  '   paragraph(s) that the surrounding narrative cites with [ref:pageN:',
  '   paragraphM] markers. Do NOT introduce technical terms in a definition',
  '   that are not already present in the cited paragraphs. Do NOT collapse',
  '   distinct concepts the prose treats as separate (e.g., if the prose',
  '   lists "CRDTs, mergeable persistent data structures, and operational',
  '   transformation" as three items, do NOT emit a single item that lumps',
  '   them under "automatic conflict resolution" — emit three items, or one',
  '   item naming only the concept the prose actually defines).',
  '   Definitions that describe a *property* (e.g., "convergent") must NOT',
  '   silently equate the property with one specific *technique* — if the',
  '   prose says "ensuring all replicas reach the same state through',
  '   techniques like X", the definition should name the property; the',
  '   technique goes in a separate item or as a parenthetical example.',
  '',
